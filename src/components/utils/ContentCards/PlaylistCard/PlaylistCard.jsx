...
const PlaylistCard = ({name, url, uploaderName, uploaderUrl, uploaderVerified, videos , thumbnail, isSearch}) => {
    const navigate = useNavigate();
    return (
        <div className="md:max-w-[720px] max-h-[8rem] md:h-[7rem] max-w-[18rem] rounded-lg md:m-2 relative flex flex-row flex-shrink-0  flex-grow  cursor-pointer  overflow-hidden group " >
        <div
          className="relative w-full h-[98%] max-w-[180px] top-0 rounded-lg mr-2 i transition-all bg-cover bg-center bg-no-repeat "
          style={{ backgroundImage: `url(${thumbnail})` }}
          onClick={() => navigate(url)}
        >
            <div className="absolute bottom-0 right-0 h-full w   w-2/4 bg-slate-800/60 "></div>
        </div>
        ...
        <div className=" flex flex-col w-full h-full pl-2 p-1">
          <div className="basis-1/4  flex ">
            <span
              ...
              onClick={() => navigate(url)}
            >
              {isSearch ? name : (name?.length > 30 ? name?.substring(0, 60) + "..." : name)}
            </span>
          </div>
  
          <div className="basis-1/4 mt-1 text-natural-800 flex flex-row justify-start group/sub" 
