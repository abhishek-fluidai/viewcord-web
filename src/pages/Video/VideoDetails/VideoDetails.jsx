...
const SaveToPlaylist = lazy(() => import("../../../components/common/SaveToPlaylist/SaveToPlaylist"));

const VideoDetails = ({ fetchedData }) => {
  const [subscription, setSubscription] = useState(null);
  const [openShare, setOpenShare] = useState(false);
  const [openSaveToPlaylist, setOpenSaveToPlaylist] = useState(false);
  ...
  return (
    <>
      <div className="video-info  w-full sm:mx-auto md:mx-4 md:mr-2 px-2 overflow-hidden mt-2">
      ...
      <Suspense fallback={<div>Loading...</div>}>
        <SaveToPlaylist
          open={openSaveToPlaylist}
          onClose={() => setOpenSaveToPlaylist(false)}
        />
      </Suspense>
    </>
  );
